...
kinematic crack algorithm, but not in \cref{fig:ex-result}. This is an
...
However, when looking at the trajectory plot in \cref{traj-wrap}, some
...
cluster location to the trajectory plot in \cref{fig:traj-wrap}, the top
...
well here. Further, \cref{fig:int-best-plots} zooms in on an example for
week 2 where our intersection method performs best (plot (a)) and linear
interpolation performs best (plot (b)). \cref{fig:int-best-plots}a) is
cluster 1, where the observed points are more spread out along the
trajectory. On the other hand \cref{fig:int-best-plots}b) has small,
