...
filter(f2, t ==7) %>%
  ggplot(aes(x = xmap, y = ymap, group = gpid, ids = gpid,
             color = clust, fill = clust)) + 
  geom_point() + ggtitle("Simulation Plot at t=7") + scale_fill_manual(values = alpha(c("blue", "orange"), .5)) + 
  facet_wrap(vars(sim))
