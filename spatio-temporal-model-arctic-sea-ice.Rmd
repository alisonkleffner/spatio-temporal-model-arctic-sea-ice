...
```{r int-picture, fig.scap= "Example of finding intersections", fig.cap= "(a). Polygons of week 1 clusters (b). Polygons of week 2 clusters (c). Intersection polygons of the overlap of week 1 and week 2 polygons", message=FALSE, warning = FALSE}
a1 <- ggplot() +
  geom_sf(data = y1, aes(fill = Group.1), alpha = 0.3) + ggtitle ("(a). Week 1 Polygons")

a2 <- ggplot() +
  geom_sf(data = y2, aes(fill = Group.1), alpha = 0.3) + ggtitle ("(b). Week 2 Polygons")

a3 <- int_plot(time_1_og,y1,y2)
...
```{r by-week-cluster-plot, fig.cap= "Results of Clustering using Bounding Box Method by Week", cache=TRUE, message=FALSE, warning = FALSE}
...
apd1_nomiss_c1 <- filter(apd1_nomiss, clust == 1)
apd2_nomiss_c1 <- filter(apd2_nomiss, clust == 1)

tmp1 <-  apd1_nomiss_c1 %>% highlight_key(~gpid)
...
tmp2 <-  apd2_nomiss_c1 %>% highlight_key(~gpid)
...
#Linear Plots Example

apd3_nomiss_c3 <- filter(apd3_nomiss, clust == 3)
apd1_nomiss_c4 <- filter(apd1_nomiss, clust == 4)

tmp3 <-  apd3_nomiss_c3 %>% highlight_key(~gpid)

p3 <- ggplot(tmp3, aes(x = xmap, y = ymap, group = gpid, 
                 hoverinfo = NULL,
                 color = factor(gpid %% 10))) + 
  geom_path(arrow = arrow(length = unit(1, "mm")), alpha = .5) + 
  scale_color_viridis_d() +
...
tmp4 <-  apd1_nomiss_c4 %>% highlight_key(~gpid)

p4 <- ggplot(tmp4, aes(x = xmap, y = ymap, group = gpid, 
                 hoverinfo = NULL,
                 color = factor(gpid %% 10))) + 
  geom_path(arrow = arrow(length = unit(1, "mm")), alpha = .5) + 
  scale_color_viridis_d() +
...
```{r int-best-plots, message=FALSE, warning = FALSE, cache=TRUE, fig.cap = "Trajectories for a) Week 1 Cluster 1 and b) Week 2 Cluster 2. These are instances where our method outperforms linear interpolation", fig.scap="Clusters for Intersection Model has lowest MSE"}
...
```{r lin-best-plots, message=FALSE, warning = FALSE, cache=TRUE, fig.cap = "Trajectories for a) Week 1 Cluster 4 and b) Week 3 Cluster 3. These are instances linear interpolation outperforms our method", fig.scap="Clusters for Linear Interpolation has lowest MSE"}

ggarrange(p4, p3, ncol = 2)
...
ice_border_clust <- data.frame(Cluster = c(1,2,3,4,5,6), X1 = c(2.38, 4.01, 2.79, 2.49, 4.16,3.19), Y1 = c(3.90, 4.34, 3.13, 3.00, 3.87, 3.30), X2 <- c(2.35, 0.19, 1.14, 1.24, 0.62, 2.21), Y2 <- c(8.33, 0.25, 3.88, 4.32, 4.55, 4.85), X3 = c(2.81, 2.94, 2.94, 3.26, 4.14," "), Y3 = c(3.04, 2.96, 3.07, 3.20, 3.88," "), X4 = c(3.82, 2.39, 1.46, 1.32, 0.43," "), Y4 = c(5.19, 0.99, 1.97, 0.54, 0.34, " "), X5 = c(2.69, 2.74, 3.49, 2.84, 2.84, 3.21), Y5 = c(2.86, 2.85, 3.31, 3.22, 2.83, 2.92), X6 = c(1.88, 1.88, 0.22, 2.25, 0.22, 0.303), Y6 = c(2.62, 1.92, 0.43, 1.46, 0.49, 0.56))
#
...
```{r cp-table-border-ou, message=FALSE, warning = FALSE}

cp_simdata2 <- data.frame(sim = c("1", "2", "3"), overx = c(0.294, 0.33, 0.352), underx = c(0.386, 0.406 , 0.371), overy = c(0.304, 0.302, 0.358), undery = c(0.352, 0.43, 0.358))

kableExtra::kable(cp_simdata2, booktabs = TRUE, col.names = c("Week", "Above Interval", "Under Interval", "Above Interval", "Under Interval"), caption = "Proportion of Interval Above/Under Actual Value for Border Values") %>% add_header_above(c(" ", "X" = 2, "Y" = 2))
