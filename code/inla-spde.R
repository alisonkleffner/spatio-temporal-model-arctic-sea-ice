...
ndex.pred <- inla.stack.index(join.stack, "stdata.pred")$data

marg3r <- res$summary.fitted.values[ndex.pred,"sd"]

inla.hpdmarginal(0.95, marg3r)
...
res$summary.fitted.values[ndex.pred, "mean"]

m = res$marginals.fixed$x

pgrid0 <- inla.mesh.projector(prmesh1, xlim = 0:1, ylim = 0:1,
                              dims = c(101, 101))

res$summary.random$i[ndex.pred-1, "mean"]

b <- pgrid0$proj$A


prd0.m <- inla.mesh.project(pgrid0, m1$summary.random$spatial.field$mean)
prd0.s <- inla.mesh.project(pgrid0,  m1$summary.random$spatial.field$sd)
## Attempt 2 -------------------------------------------------------------------
...
###########Attempt 5 (Spatial Model with Time Fixed Effect)
val2 <- data_inla(f_sim2, 3, polyg2, w2, n2, 2) #Get Data set up properly (t=3, int = 1)
val2_known <- filter(val2, !is.na(xmap))
